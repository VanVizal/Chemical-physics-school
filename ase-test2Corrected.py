...
def test_molecule(mol_name, method='RHF', basis='6-31g', mopac_method='PM7'):
    ...
    return energy_pyscf, energy_mopac
...
if __name__ == "__main__":
    ...
    # Dictionary to store results: {mol_name: (energy_pyscf, energy_mopac)}
    results = {}

    # Test each molecule
    for mol_name in test_molecules:
        ep, em = test_molecule(mol_name)
        results[mol_name] = (ep, em)
    ...
    for mol_name, (ep, em) in results.items():
        # If energy is None, print N/A
        ep_str = f"{ep:.4f}" if ep is not None else "N/A"
        em_str = f"{em:.4f}" if em is not None else "N/A"
        print(f"{mol_name:<10} {ep_str:<20} {em_str:<20}")
